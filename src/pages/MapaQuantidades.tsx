        ...
        workbook.SheetNames.forEach((sheetName, index) => {
            ...
            if (Array.isArray(row)) {
              for (let j = 0; j < row.length; j++) {
                ...
                if (cellValue === "ARTIGO" || cellValue.includes("ARTIGO") ||
                  cellValue === "CAP." || cellValue === "CAP" ||
                  cellValue === "POS." || cellValue === "POS" ||
                  cellValue === "COD." || cellValue === "COD" ||
                  cellValue === "POSIÇÃO" || cellValue === "POSICAO") {
                  artigoColumnIndex = j;
                }
                if (cellValue === "DESCRIÇÃO" || cellValue.includes("DESCRIÇÃO") ||
                  cellValue === "DESCRICAO" || cellValue.includes("DESCRICAO") ||
                  cellValue === "DESIGNAÇÃO" || cellValue === "DESIGNACAO" ||
                  cellValue.includes("DESIGN.")) {
                ...
                if (cellValue === "QT" || cellValue === "QUANTIDADE" || cellValue === "QUANT." ||
                ...
                if (cellValue.includes("PRECO") || cellValue.includes("PREÇO") ||
                  ...
                  cellValue === "PREÇO" || cellValue === "PRECO") {
          ...
          if (artigoColumnIndex !== -1 && descricaoColumnIndex !== -1) {
            ...
            jsonData.forEach((row: unknown, rowIndex: number) => {
              ...
              // Case 1: Chapter (pure number or single letter in ARTIGO column)
              if (/^(\d+|[A-Z])$/.test(artigoCell) && descricaoCell) {
              ...
              // Article is a row with ARTIGO containing one or more dots (e.g., "1.1", "1.2.3")
              ...
              else if (artigoCell && /^(\d+|[A-Z])(\.\d+)*\.?$/.test(artigoCell) && !hasUN && !hasQT && descricaoCell) {
              ...
              // They will be displayed to the user for manual accept/reject decision
                ...
                if (!artigoCell) {
                  // If it looks like a note, it shouldn't be uncertain, it's just contextual text
                  if (descricaoCell.toUpperCase().startsWith("NOTA") || descricaoCell.toUpperCase().startsWith("OBS")) {
                    return; // Skip adding to uncertain rows
                  }
                  uncertainReason = 'Row has description but no article number and is not in a recognizable context';
                } else if (!hasUN && !hasQT) {
                  // If ARTIGO is text like "Nota 1", it's not uncertain, it's just descriptive
                  if (artigoCell.toUpperCase().startsWith("NOTA") || artigoCell.toUpperCase().startsWith("OBS")) {
                    return; // Skip adding to uncertain rows
                  }
